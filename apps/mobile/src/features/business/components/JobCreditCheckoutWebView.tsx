...
import { startJobCreditCheckout, getJobCreditBalance } from "../../../services/jobCreditApi";
import type { BillingInfoInput } from "../../../services/subscriptionApi";

interface JobCreditCheckoutWebViewProps {
...
export function JobCreditCheckoutWebView({ startingBalance, onDone }: JobCreditCheckoutWebViewProps) {
  ...
  async function attemptCheckout(fields: BillingInfoInput) {
    ...
    setError(null);
    try {
      const session = await startJobCreditCheckout(fields);
  ...
  async function pollForCompletion() {
    ...
    for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        const { balance } = await getJobCreditBalance();
    ...
    <View style={styles.webviewContainer}>
      <WebView
        ...
        onNavigationStateChange={(navState) => {
          if (navState.url.includes("/payments/iyzico/job-credit-callback")) {
            pollForCompletion();
          }
        }}
      />

...
export function CheckoutWebView({ planCode, onDone }: CheckoutWebViewProps) {
  ...
  async function attemptCheckout(fields: BillingInfoInput) {
    ...
    setError(null);
    try {
      const session = await startCheckout(planCode, fields);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 400) {
        ...
        setError(getApiErrorMessage(err, "Abonelik başlatılamadı"));
      }
    } finally {
      setLoading(false);
    }
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          Bu bilgiler bir kere istenir, sonraki aboneliklerde tekrar sorulmaz.
        </Text>
        <Input
          style={styles.field}
    ...
    <View style={styles.webviewContainer}>
      <WebView
        ...
        onNavigationStateChange={(navState) => {
          if (navState.url.includes("/payments/iyzico/callback")) {
            pollForCompletion();
          }
        }}
      />
